# GaussCluster
**A 3D Astrometric Gaussian Mixture Model Pipeline for Open Cluster Extraction**
...
`GaussCluster` solves this by operating strictly in 3D probabilistic Astrometric space (proper motion $\mu_{\alpha*}$, $\mu_{\delta}$, and parallax $\varpi$). It isolates the true dynamical members of a cluster from dense galactic backgrounds, completely independent of photometric priors. 
...
- **Anchor the Astrometric Core:** Utilize a 3D sigma-clipping algorithm to establish a systemic cluster centroid.
...
| **Berkeley 87** | 70 ($P \ge 0.9$) | Highly complex, differential reddening target (Cygnus X). Pipeline bypassed massive spatial contamination to recover a clean Astrometric Main Sequence. |
...
2. **Astrometric Catalogue (`.csv`):** A clean data table containing all Gaia DR3 astrometry and photometry for stars meeting the strict $P \ge 0.9$ Astrometric membership threshold. 
